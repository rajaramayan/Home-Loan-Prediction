...
# ── Page Config ──────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="Home Loan Prediction", page_icon="🏠", layout="wide"
)
...
def preprocess_and_train():
    ...
    df = df.drop(
        [
            'Gender', 'MartialStatus', 'Age', 'Education',
            'LoanType', 'LoanStatus',
        ],
        axis=1
    )
    ...
    df['MaxLoanAmount'] = df.apply(
        lambda r: calc_max_loan(
            r['MaxEmi'], r['InterestRate'], r['Tenure']
        ),
        axis=1
    )
    ...
    iqr = p75 - p25
    df1 = df[
        (df['HomeValue'] >= p25 - 1.5 * iqr)
        & (df['HomeValue'] <= p75 + 1.5 * iqr)
    ]

    # Remove TotalIncome outliers
    p25 = df1['TotalIncome'].quantile(0.25)
    p75 = df1['TotalIncome'].quantile(0.75)
    iqr = p75 - p25
    nf = df1[
        (df1['TotalIncome'] >= p25 - 1.5 * iqr)
        & (df1['TotalIncome'] <= p75 + 1.5 * iqr)
    ]

    key_features = ['HomeValue', 'InterestRate', 'Tenure', 'TotalIncome',
                    'MaxEmi', 'MaxLoanAmount', 'EligibleLoanAmount']
    log_features = [
        'HomeValue', 'TotalIncome', 'MaxEmi', 'MaxLoanAmount',
        'EligibleLoanAmount'
    ]
    ...
    for name, model in model_dict.items():
        ...
        cv_r2 = cross_val_score(
            model, X_transform, y, cv=k, scoring='r2'
        )
        cv_mae = cross_val_score(
            model, X_transform, y, cv=k,
    ...
    results_df = (
        pd.DataFrame(results).T.sort_values('CV Mean R²', ascending=False)
    )
    ...
    return (
        df_raw, nf, results_df, model_dict['Random Forest'],
        predictions, y_test, y_test_binned, pred_binned
    )
...
if page == "📊 EDA":
    ...
    st.markdown(
        f"Dataset: **{len(df_raw)} rows × {len(df_raw.columns)} columns**"
    )
    ...
    with col8:
        fig, ax = plt.subplots()
        df_raw['TotalIncome_temp'] = (
            (df_raw['ClientIncome'] + df_raw['FamilyIncome']) / 12
        )
...
# ═════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown(
        "Models trained on **80%** of data after outlier removal "
    ...
    st.dataframe(
        results_df.style
        .highlight_max(
            subset=['Test R²', 'CV Mean R²'], color='#c6efce'
        )
        .highlight_min(
            subset=['Test RMSE', 'Test MAE', 'CV Mean MAE', 'CV Mean RMSE'],
            color='#c6efce'
        ),
    ...
    best_r2 = results_df.loc[best, 'CV Mean R²']
    st.success(
        f"✅ Best Model: **{best}** (highest CV Mean R²: {best_r2})"
    )
    ...
    plt.suptitle(
        "Actual vs Predicted Loan Amount", fontsize=14, fontweight='bold'
    )
    plt.tight_layout()
    st.pyplot(fig)
    plt.close(fig)
    ...
    for idx, name in enumerate(model_names):
        ...
        ax.scatter(
            predictions[name], residuals,
            alpha=0.5, color='darkorange', s=25
        )
    ...
    st.subheader(
        "🟩 Confusion Matrices — "
        "Loan Amount Category (Low / Medium / High)"
    )
    st.caption(
        "Since this is a regression problem, predicted loan amounts "
        "are binned into **Low** (bottom 33%), **Medium** (middle 33%), "
        "and **High** (top 33%) "
    ...
    for idx, name in enumerate(model_names):
        ax = axes[idx]
        cm = confusion_matrix(
            y_test_binned, pred_binned[name], labels=class_labels
        )
        ...
        ax.set_title(name)
    plt.suptitle(
        "Confusion Matrices — Loan Amount Category",
    ...
    st.dataframe(
        acc_df.style.background_gradient(
            subset=['Precision', 'Recall'], cmap='Greens'
        ),
        use_container_width=True
    )
    ...
    st.markdown(
        "The chart below shows how much each input feature contributed to the "
        "Random Forest model's predictions, measured by the average reduction "
        "in impurity (variance) across all trees."
    )
    ...
    with col_chart:
        ...
        palette = plt.cm.RdYlGn(fi_df['Importance'] / max_imp)
        bars = ax.barh(
            fi_df['Feature'][::-1],
            fi_df['Importance %'][::-1],
            color=palette[::-1],
    ...
    with col_table:
        st.markdown("**Ranked feature contributions**")
        st.dataframe(
            fi_df[['Feature', 'Importance %']]
            .style.background_gradient(subset=['Importance %'], cmap='YlGn'),
            use_container_width=True,
            hide_index=True
        )
    ...
    for _, row in fi_df.iterrows():
        st.markdown(
            f"- **{row['Feature']}** "
            f"contributes **{row['Importance %']:.2f}%** to the prediction"
        )
...
# ═════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown(
        "Enter the applicant's details below to get an estimated "
        "loan amount using the **Random Forest** model."
    )
    ...
    if submitted:
        ...
        input_df = prepare_input(
            home_value, total_income, interest_rate, tenure
        )
        ...
        r1.metric("🏦 Predicted Loan Amount", f"रू{prediction:,.0f}")
        r2_col.metric("🏡 Home Value", f"रू{home_value:,.0f}")
        ...
        c1.metric("Monthly Total Income", f"रू{total_income:,.0f}")
        c2.metric("Max EMI Capacity", f"रू{max_emi:,.0f}")
        c3.metric("Max Loan (EMI-based)", f"रू{max_loan:,.0f}")
        c4.metric("Eligible Loan (70% LTV)", f"रू{eligible:,.0f}")

        st.info(
            f"The model predicts a loan amount of **रू{prediction:,.0f}** "
            f"for a property valued at **रू{home_value:,.0f}** "
            f"with a **{tenure}-month** tenure "
            f"at **{interest_rate}%** annual interest."
        )
